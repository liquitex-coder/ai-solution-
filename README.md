...
│ POST /audit; AINAVI_GATE_MODE: report_only | canary | full │
└─────────────────────────────┬───────────────────────────────┘
                              ▼
┌─────────────────────────────────────────────────────────────┐
│ WordPress REST API                                            │
...
| auditor | internal: http://auditor:8090 (no host port) | Auditor Gate HTTP service |
...
| `AINAVI_GATE_MODE` | `report_only` (default), `canary`, or `full` |
...
| auditor | internal: http://auditor:8090（ホストポートなし） | AuditorゲートのHTTPサービス |
...
| `AINAVI_GATE_MODE` | `report_only`（既定）、`canary`、`full` |
