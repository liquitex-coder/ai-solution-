...
## 16. §8 言語戦略の改訂（2026-09-25・仕様のみ）
- 操作者の指示「§8言語戦略の改訂を進めて」を受けて、要件 §8 を v2.8 に改訂した（`2184ca1`）
- 決めたこと
  - 原本は日本語。他の言語版は、Auditor を PASS した原本の翻訳
  - 表示言語の優先順: 切替スイッチで選んだ言語 → ブラウザの言語設定 → 日本語
  - 強制リダイレクトはしない。言語ごとの URL（`/en/` など）、hreflang、案内バナーを使う
  - 段階展開: JA → EN → ES/FR。昇格には監査ルール・忠実性チェック・手動実行ログ・署名が必要
- 一次情報で確認したこと
