...
SPEC (read docs/requirements.md §28 first; it is the contract):
  - http.server.ThreadingHTTPServer; bind AINAVI_GATE_BIND (default 0.0.0.0), port AINAVI_GATE_PORT (default 8090).
  - GET /health -> 200 {"status":"ok","service":"ainavi-auditor-gate","memory_db":<bool: db file openable>}
  - POST /audit body {content, source_urls?, skill_ref?, title?} -> 200 {"verdict","reasons","skill_ref","audited_at","fact_id"}.
    ...
    content_hash = sha256(content). DB path from AINAVI_GATE_MEMORY_DB (default data/memory.db); ensure_schema on startup.
...
ACCEPTANCE:
  python3 -m unittest discover -s tests -v                              # exit 0
  (AINAVI_GATE_PORT=8090 python3 scripts/auditor_server.py & sleep 1; curl -s localhost:8090/health; kill %1)   # {"status":"ok",...}
...
```
GOAL: Wire the Auditor service into the local sandbox so n8n's Code nodes see AINAVI_GATE_URL (requirements §28-3).
...
SPEC:
  - Add service `auditor`: image python:3.11-slim, working_dir /app, volumes ./scripts:/app/scripts:ro and ./data:/app/data,
    environment AINAVI_GATE_MEMORY_DB=/app/data/memory.db, AINAVI_GATE_PORT=8090, command ["python3","scripts/auditor_server.py"], expose 8090 (no host port needed), healthcheck via python3 -c urllib GET /health.
  - n8n service: add environment AINAVI_GATE_URL=http://auditor:8090, AINAVI_GATE_MODE=${AINAVI_GATE_MODE:-report_only}, N8N_BLOCK_ENV_ACCESS_IN_NODE=false; depends_on auditor.
  - .env.example: add a commented block "# Claim-Auditor gate rollout mode (requirements §22-3): report_only | canary | full" and AINAVI_GATE_MODE=report_only.
ACCEPTANCE:
  docker compose config >/dev/null      # valid (skip if docker absent, say so)
  grep -n "AINAVI_GATE_URL=http://auditor:8090" docker-compose.yml
...
SPEC:
  ...
  - W9: FAIL unless scripts/auditor_server.py exists AND docker-compose.yml contains a service named auditor AND the n8n service environment contains AINAVI_GATE_URL. Parse docker-compose.yml with a minimal line-based scan (no PyYAML).
...
SPEC:
  ...
  - Environment variables table: add AINAVI_GATE_MODE, KIMI_API_KEY, FAL_API_KEY, WP_BEARER_TOKEN/WP_SITE (production, §24).
...
SPEC:
  ...
  - New Step 4b "Claim-Auditor gate variables": AINAVI_GATE_URL (public HTTPS of the §28 service; local sandbox uses http://auditor:8090 automatically), AINAVI_GATE_MODE=report_only.
  ...
  - n8n_deploy.ps1: remove the last reminder bullet about the YouTube auth mismatch (resolved, §25-4); add a reminder to set AINAVI_GATE_URL / AINAVI_GATE_MODE.
ACCEPTANCE:
  ! grep -n "即公開" n8n/SETUP_GUIDE.md ; grep -n "AINAVI_GATE_URL" n8n/SETUP_GUIDE.md scripts/n8n_deploy.ps1
...
After T-10 is merged, ask Codex to patch `n8n/workflows/*.json` (insert "図解埋め込み" Code node between WP整形 and Auditor Gate calling `${auditorUrl}/embed-diagrams`; add `const auditorUrl = $env.AINAVI_GATE_URL || ($vars && $vars.AINAVI_GATE_URL) || '';` fallback in every gate; allow one ```mermaid fence in `n8n/prompts/article-base.md`). Then **run WF07 manually in n8n**, confirm a WordPress draft with a Kroki image and a non-SKIP verdict, paste the execution log into the PR, and only then push (CLAUDE.md §F).
