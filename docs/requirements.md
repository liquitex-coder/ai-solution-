...
| 本番（n8n cloud） | **Fly.io** — T-26 の `Dockerfile` + `fly.toml`（1GB `data` volume、`internal_port: 8090` ピニング）。Fly CLI: `fly deploy -c fly.toml`。環境変数 `AINAVI_GATE_TOKEN` は `fly secrets set AINAVI_GATE_TOKEN=<value>` で設定。Fly app 名: `ainavi-auditor-gate`（Claim-Auditor リポジトリとの混同を避けるため `claim-auditor` から改名・2026-09-16）。ホスト: `https://ainavi-auditor-gate.fly.dev`（自動 HTTPS）。決定日: 2026-09-13、**デプロイ完了: 2026-09-17**（region `nrt`、volume `vol_40o0mnm90wnn5qk4` 1GB、`GET /health` → `{"status":"ok","auth":true,"memory_db":true}` 確認済み） | `AINAVI_GATE_URL` / `AINAVI_GATE_MODE=report_only` / `AINAVI_GATE_TOKEN` / `GITHUB_TOKEN` は n8n cloud の **Variables（`$vars`）** に設定済み（T-13, 2026-09-20）。ゲートは `$vars` → `$env` の順で解決する（`cfg()`、要件 §32-1 D10）。**T-13 実測3点（2026-09-20、使い捨てCodeノード、n8n cloud 上）**: (1) `$vars.AINAVI_GATE_URL` 読取 → `true`（`$env.*` は例外、想定通り）。(2) `fetch` → `ReferenceError: fetch is not defined`（**未対応と判明**）。(3) `require('crypto')` → `sha256('abc')` 一致確認。追加確認: `this.helpers.httpRequest({method:'GET',url:'.../health',json:true})` で `/health` に到達し `{status:"ok",auth:true,memory_db:true}` を取得（n8n cloud から Fly.io への到達性を T-12 とは別経路で再確認）。→ `fetch` 不使用の方針を §32-1 D10 に記録し、全9ワークフローの「プロンプト読込み」`fetchPrompt()` と「Auditor Gate」ノードを `this.helpers.httpRequest` に置換済み（2026-09-20、`scripts/patch_evidence_pack.py` の `_gate_js()` も同様）。WF09「9媒体横断取材」ノードと `_probes_js()` の URL 生死判定は未対応（T-35b/T-36b で実機確認してから移植） |
...
| D10 | ワークフロー JSON の生成手段 / Code ノードの `$env` / Code ノードの HTTP 手段 | `scripts/patch_workflows.py`（2026-07-10 の一回限りパッチ）/ 全 Code ノードの `$env.*` 参照 / 全 Code ノードの `fetch()` 呼び出し | **再実行禁止**: `patch_workflows.py` はゲートノードを二重追加し `AINAVI_GATE_MODE`・`category_id` を持たない旧コードで上書きする（W7/W11 退行）。n8n 2.x は既定で Code ノードの `$env` を遮断し、参照は例外になる（T-13 実測で確認: `$vars.AINAVI_GATE_URL` 読取可 / `$env.*` は `access to env vars denied` で例外）。**`fetch` は n8n cloud の Code ノードに存在しない（T-13 実測 2026-09-20: `ReferenceError: fetch is not defined`）**。代わりに `this.helpers.httpRequest({ method, url, headers, body, json })` が動作することを実測確認済み（同ノードから `GET /health` → `{status:"ok",auth:true,memory_db:true}` を取得）。`require('crypto')` は利用可（`sha256` 一致確認済み）、`process` は不可（`process is not defined`） | T-35a: `patch_workflows.py` に実行ガード、`scripts/patch_evidence_pack.py`（冪等）を正とする。全 Code ノードは `cfg()`（`$vars` → `$env`、各 try/catch）で設定を読む。**2026-09-20: `fetch` 不使用の原則を追加** — `_gate_js()` と全9ワークフローの「プロンプト読込み」`fetchPrompt()`／「Auditor Gate」ノードを `this.helpers.httpRequest` に置換済み（HTTP ステータス個別判定はせず、非2xx・ネットワーク例外は一律 catch して `UNVERIFIABLE`/空文字列扱い）。2026-09-20 追加実測: `this.helpers.httpRequest` は非2xxで例外を投げ、HTTPステータスは `e.status`（axios準拠、`e.statusCode` ではない）に乗る（実機確認: 存在しないパスへのGETで `e.status === 404`）。これに基づき `_probes_js()`（URL生死判定・GitHubリポジトリ存在確認）と WF09「9媒体横断取材」ノード（GitHub検索・HN・Reddit・RSS・note・YouTube）も `this.helpers.httpRequest` に置換済み（`fetch` 呼び出しはリポジトリ全体で0件）。`tests/test_evidence_nodes_js.py` は `this.helpers.httpRequest` を `.call()` でスタブする方式に更新（`fetch` グローバルの直接定義は廃止） |
| D11 | HTTP Request ノード（`n8n-nodes-base.httpRequest` v4.2）のパラメータ形式 | 全ワークフローの HTTP ノードが `"headers": {"parameters": [...]}` / `"body": {"contentType": "json", "jsonBody": {…オブジェクト…}}` という**入れ子形式**で記述 | **n8n に存在しないスキーマ**（2026-09-21、T-35b の WF01 手動実行で判明: Claude ノードが `JSON Body is not valid JSON`、GitHub ノードが空クエリで 422）。n8n 本体 `HttpRequestV3.node.ts` で確認した実スキーマは**トップレベル**の `sendQuery`/`specifyQuery`/`queryParameters.parameters`、`sendHeaders`/`specifyHeaders`/`headerParameters.parameters`、`sendBody`/`contentType`/`specifyBody`/`jsonBody`（**文字列**テンプレート）。トップレベル `body` は raw 文字列ボディ用の別パラメータで、入れ子の `body` オブジェクトは無視される。`queryParameters` だけは偶然一致していた。`scripts/patch_evidence_pack.py` の Fact-Check Verifier ノードも同じ入れ子形式だった。WF08 Kimi ノードの `content: "{{ $json.promptContent }}"`（`=` 無し）はこの形式では式として評価されず文字列のまま送られていた | 2026-09-21 一括変換: `headers` → `headerParameters`、`body.jsonBody`（オブジェクト）→ `jsonBody`（`=` 始まりの JSON テンプレート文字列。式の値は `{{ JSON.stringify((式)) }}` で埋め込み、混在テンプレートは JS テンプレートリテラルに変換）。変換器は `scripts/patch_evidence_pack.py` の `json_body_template()` / `convert_http_node()` に置き、Verifier ノードも同じ経路で生成。`tests/test_workflow_http_schema.py` が全 HTTP ノードに入れ子 `headers`/`body` が無いことをセンサーとして監視（ラチェット、CLAUDE.md §C-6）。**実装済み（2026-09-21、T-35b 第1回手動実行で確認）**: `n8n_deploy.ps1 -Update -Only 01` 後の WF01 実行で GitHub 検索・Claude 生成・Fact-Check Verifier・Auditor Gate の各 HTTP ノードが実リクエストを送信（Auditor は `fact_id: 1` を返却）。残る失敗は下記 D12〜D14 と WordPress 401 |
| D12 | Fact-Check Verifier の `output_config.format.schema`（`VERIFIER_SCHEMA.feasibility`） | `{"type": ["string","null"], "enum": [..., null]}` | **Anthropic に拒否（T-35b 第1回実行、2026-09-21）**: `400 invalid_request_error: Enum value 'PLAUSIBLE' does not match declared type ['string','null']` → `verifier.ok=false`、`claims=[]`、`WARN:FACTCHECK_UNAVAILABLE`。構造化出力の JSON Schema は `enum`/`anyOf` をサポートするが、型配列との併用は不可 | `feasibility` を `{"anyOf": [{"type":"string","enum":[PLAUSIBLE,IMPLAUSIBLE,UNKNOWN]}, {"type":"null"}]}` に変更（`content_audit.py` 側の受理値は不変）。`tests/test_patch_evidence_pack.py` が「`enum` と型配列の併用なし」をセンサーとして監視 |
| D13 | Evidence Probes の URL 抽出（§34-3） | `https?:\/\/[^\s"'<>)]+` — ASCII の `)` は除外するが全角句読点は除外しない | **誤検出（T-35b 第1回実行）**: 記事中の `（https://github.com/openclaw/openclaw）` から末尾の `）` を含めて抽出し、`404 → DEAD` → `WARN:URL_DEAD`、`probes_failed: 1`。日本語記事では常態化する | 抽出後に末尾の `）」』】〉》、。，．！？!?:;,.` を剥がす（`_probes_js`）。JS ハーネステストに全角括弧ケースを追加 |
| D14 | 記事の見出し形式（`FAIL:STRUCTURE`、§22-1） | `n8n/prompts/00-copyright-transform.md`「3つ以上の `##` 見出しブロック」（Markdown） vs `article-base.md` / `01-github-trending.md`「`<h2>` タグ」（HTML）。監査 `H2_RE` は `<h2>` を数える | **プロンプト間の矛盾（T-35b 第1回実行）**: 生成記事が Markdown 見出し（`## `）で出力され `FAIL:STRUCTURE:h2=0<3`。全WF共通で先に読み込まれる著作権プロンプトの指示が優先された | `00-copyright-transform.md` を `<h2>`（HTML）に統一。監査側は HTML 契約（WordPress REST が HTML を受ける）のまま変更しない。プロンプトは各WFの「プロンプト読込み」ノードが GitHub の**既定ブランチ（main）**から取得するため、main へのマージ後に有効になる |
