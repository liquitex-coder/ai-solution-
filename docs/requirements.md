...
6. 翻訳に使うモデルとコスト上限 → **モデルは決定済み（2026-09-25）**: `claude-sonnet-5`（記事生成の WF06/07/09 と同じ）。翻訳は急がないため Batch API（半額）を第一候補とする。1記事あたりの費用は推定値（入力 約5,000・出力 約2,500トークンで 約$0.035）で、T-56 の実装時にトークン計数 API で実測して確定する。**月の上限額は未決**（上限を超えた日は翻訳を止め、下書きのままにする）
