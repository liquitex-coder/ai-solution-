...
- 多言語プラグイン Polylang の「ブラウザ言語の検出」は、**初回訪問時にトップページだけ**を自動リダイレクトする（polylang.pro の公式ガイドで確認）。上の原則と衝突するため、**既定は OFF** とし、ON にするかは操作者の判断とする（8-7）。
...
1. WordPress.com のプランが有料かどうか（多言語プラグインの前提。§35-14 と同じ確認）
