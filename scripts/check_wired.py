...
Checks:
  ...
  W2  every workflow has an Auditor Gate node (AUDITOR_GATE_URL or, until T-11, CLAIM_AUDITOR_URL)
  ...
  W7  every Auditor Gate is rollout-mode aware (AUDITOR_GATE_MODE / CLAIM_AUDITOR_MODE)
  W8  every Auditor Gate skill_ref is registered for ratchet prompts
  W9  Auditor service and n8n AUDITOR_GATE_URL wiring exist (§34)
...
def main() -> int:
    ...
    for name, wf in workflows.items():
        ...
        # §34 transition: gate nodes keep $env.CLAIM_AUDITOR_* until T-11 rewrites them.
        if "AUDITOR_GATE_URL" in blob or "CLAIM_AUDITOR_URL" in blob:
            ok(f"W2 {name}: Auditor Gate present")
        else:
            fail(f"W2 {name}: no Auditor Gate node (AUDITOR_GATE_URL not found)")

        if "AUDITOR_GATE_MODE" in blob or "CLAIM_AUDITOR_MODE" in blob:
            ok(f"W7 {name}: gate is rollout-mode aware")
        else:
            fail(f"W7 {name}: Auditor Gate is not AUDITOR_GATE_MODE aware (§22-3)")
    ...
    if compose_path.exists():
        for line in compose_path.read_text(encoding="utf-8").splitlines():
            ...
            if in_n8n_environment and re.match(r"^    [^ ]", line):
                in_n8n_environment = False
            if in_n8n_environment and "AUDITOR_GATE_URL" in line:
    ...
    if not auditor_server.exists():
        ...
        fail("W9 docker-compose.yml: n8n environment lacks AUDITOR_GATE_URL (§34)")
    else:
        ok("W9 auditor service and n8n AUDITOR_GATE_URL wired")
